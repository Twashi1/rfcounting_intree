...
def main():
    ...
    parser.add_argument(
        "--var_freq",
        type=str,
        help="Are results being calculated under variable frequency operation"
    )
    args = parser.parse_args()

    var_freq = args.var_freq.lower() in ("true", "yes", "y")
    ...
    heat_df["temp_area_weighted_mean_etc"] = heat_df["temp_area_weighted_mean_etc"] - 273.15
    heat_df["temp_area_weighted_mean_baseline"] = heat_df["temp_area_weighted_mean_baseline"] - 273.15
    heat_df = heat_df[
        ["block_id", "temp_max_etc", "temp_area_weighted_mean_etc", "temp_area_weighted_mean_baseline", "temp_max_baseline", "temp_diff", "cycle_count"]
    ].copy()
    ...
    # TODO: get average temperature from heat df
    average_temperature = (
        heat_df["temp_area_weighted_mean_etc"] * heat_df["execution_time"]
    ...
    with open("efficiencyStats.txt", "a") as f:
        ...
        if var_freq:
            ...
            f.write(f"EDP@ETCValue: {edp_potential_etc:.6f}\n")
            f.write(f"IPS@ETCValue: {ips_potential_etc:.6f}\n")
        else:
            ...
            f.write(f"EDP@ETCValue: {edp_constant_etc:.6f}\n")
            f.write(f"IPS@ETCValue: {ips_conservative_etc:.6f}\n")

        f.write(f"EDP@BaseValue: {edp_baseline:.6f}\n")
        f.write(f"IPS@BaseValue: {ips_baseline:.6f}\n")
