...
static double SIM_crossbar_in_cap(double wire_cap, u_int n_out, double n_seg, int connect_type, int trans_type, double *Nsize)
{
	...
	if (connect_type == TRANS_GATE) {
		...
		#else
			Ctrans = ceil(nsize) * CapDrainNFET;
			if (trans_type == NP_GATE)
	 			Ctrans += ceil(psize) * CapDrainPFET;
		#endif
	}
		...
		#else
			Ctrans = Woutdrvnandn * CapInNAND + Woutdrvnorn * CapInNOR;
		#endif
	}
	...
	if (n_seg > 1) {
		Ctotal *= (n_seg + 1)/(n_seg * 2);
			...
			Ctotal += (n_seg + 2)*(n_seg - 1)/(n_seg * 2)*(Woutdrvnandn * CapInNAND + Woutdrvnorn * CapInNOR);
			/* output capacitance of tri-state buffer */
			Ctotal += (n_seg - 1) / 2 * (Woutdrivern * CapDrainNFET + Woutdriverp * CapDrainPFET);
		#else
		/* input capacitance of tri-state buffer */
	...
	#if(PARM(TECH_POINT) == 14)
		Ctotal += ceil(nsize) * (CapDrainNFET + CapGateNFET) + ceil(psize) * (CapDrainPFET + CapGatePFET);
	#else
	Ctotal += SIM_draincap(nsize, NCH, 1) + SIM_draincap(psize, PCH, 1) + SIM_gatecap(nsize + psize, 0);
	#endif
	return Ctotal / 2;
}
...
static double SIM_crossbar_out_cap(double length, u_int n_in, double n_seg, int connect_type, int trans_type, double *Nsize)
{
	...
	if (connect_type == TRANS_GATE) {
		...
		#if(PARM(TECH_POINT) == 14) 
		Ctrans = Woutdrivern * CapDrainNFET + Woutdriverp * CapDrainPFET;
		#else
	...
	#if(PARM(TECH_POINT) == 14)
		Ctotal += Woutdrivern * (CapDrainNFET + CapGateNFET) + Woutdriverp * (CapDrainPFET + CapGatePFET);
	#else
...
{	
	...
	Ctotal += SIM_draincap(Woutdrivern, NCH, 1) + SIM_draincap(Woutdriverp, PCH, 1) +
		SIM_gatecap(Woutdrivern + Woutdriverp, 0);
	#else
	Ctotal 	= CC3metal * length												// wire capacitance
			+ 2 * (Woutdrvnandn * CapInNAND + Woutdrvnorn * CapInNOR) 		// part 2
			+ 2 * (Woutdrivern * CapDrainNFET + Woutdriverp + CapDrainPFET)	// part 3
			+ ceil(nsize) * (CapDrainNFET + CapGateNFET) + ceil(psize) * (CapDrainPFET + CapGatePFET) // part 4
			+ Woutdrivern * (CapDrainNFET + CapGateNFET) + Woutdriverp * (CapDrainPFET + CapGatePFET); // part 5
	#endif
...
static double SIM_crossbar_int_cap(u_int degree, int connect_type, int trans_type)
{
	...
	if (connect_type == TRANS_GATE) {
		#if(PARM(TECH_POINT) == 14)
		Ctrans = Wmemcellr * CapDrainNFET;
		if (trans_type == NP_GATE)
			Ctrans += ceil(Wmemcellr * Wdecinvp / Wdecinvn) * CapDrainPFET;
		...
		Ctotal += degree * (Woutdrivern * CapDrainNFET + Woutdriverp * CapDrainPFET);

		/* part 2: gate cap of tri-state gate */
		Ctotal += Woutdrvnandn * CapInNAND + Woutdrvnorn * CapInNOR ;
		#else
		/* part 1: drain cap of tri-state gate */
...
static double SIM_crossbar_ctr_cap(double length, u_int data_width, int prev_ctr, int next_ctr, u_int degree, int connect_type, int trans_type)
{
	...
	if (connect_type == TRANS_GATE) {
		#if(PARM(TECH_POINT) == 14)
			Cgate = Wmemcellr * CapGateNFET;
			if (trans_type == NP_GATE)
				Cgate += ceil(Wmemcellr * Wdecinvp / Wdecinvn) * CapGatePFET;
		#else
		/* FIXME: Wmemcellr and resize */
		...
		#if(PARM(TECH_POINT) == 14)
		Cgate = Woutdrvnandn * CapInNAND + Woutdrvnorn * CapInNOR ; 
		#else
	...
	if (!(connect_type == TRANS_GATE && trans_type == N_GATE && !prev_ctr))
		Ctotal += Wdecinvn * (CapInINV + CapOutINV); //part 3
	if (prev_ctr)
		Ctotal += degree * WdecNORn * CapOutNOR;		// part 4
	if (next_ctr)
		Ctotal += (degree * 40 + 20) *WdecNORn * CapInNOR;	// part 5	
	#else
	/* part 3: inverter */
	if (!(connect_type == TRANS_GATE && trans_type == N_GATE && !prev_ctr))
	...
	if ((crsbar->model = model) && model < CROSSBAR_MAX_MODEL) {
		...
		switch (model) {
			case MATRIX_CROSSBAR:
				...
				#if(PARM(TECH_POINT) == 14)
					I_static = (Woutdrvnandn * NAND2_TAB[0] + Woutdrvnorn * NOR2_TAB[0] + Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) * n_in * n_out * data_width /* tri-state buffers */
							 + (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in * data_width			/* input driver */
							 + (Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) * n_out * data_width		/* output driver */
							 + (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in * n_out;				/* control signal inverter */	
				#endif
			...
			case MULTREE_CROSSBAR:
				...
				#if(PARM(TECH_POINT) == 14)
					I_static = (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in * data_width 			/* input driver */
							 + (Woutdrivern * NMOS_TAB[0] + Woutdriverp * PMOS_TAB[0]) * n_out * data_width		/* output driver */
							 + (WdecNORn * NOR2_TAB[0]) * (2 * n_in - 1) * n_out * data_width					/* mux */
							 + (Wdecinvn * NMOS_TAB[0] + Wdecinvp * PMOS_TAB[0]) * n_in * n_out;				/* control signal inverter */	
				#endif
...
double SIM_crossbar_stat_energy(SIM_crossbar_t *crsbar, int print_depth, char *path, int max_avg, double n_data)
{
	...
	switch (crsbar->model) {
		...
		case MULTREE_CROSSBAR:
			...
			//fprintf(stdout, "E_XBCout:%g\tE_XBCin:%g\tXBCctr:%g\tI_static: %g\tn_data: %g\tcrsbar->data_width: %d\n", crsbar->e_chg_out, crsbar->e_chg_in, crsbar->e_chg_ctr,crsbar->I_static, n_data, crsbar->data_width);
			//fprintf(stdout, "Eout: %g\tEin: %g\tEctr: %g\tE_static: %g\tPdyn (uW): %g\tPleak (uW): %g\tPtot (uW): %g\n", crsbar->e_chg_out * crsbar->data_width * (max_avg ? 1 : 0.5) * n_data, crsbar->e_chg_in * crsbar->data_width * (max_avg ? 1 : 0.5) * n_data, crsbar->e_chg_ctr * n_data, Estatic , Eavg*PARM(Freq)*1e6, crsbar->I_static*1e6, (Eavg*PARM(Freq)+crsbar->I_static)*1e6);
			break;
