...
double SIM_total_clockEnergy(SIM_router_info_t *info, SIM_router_power_t *router)
{
		...
		if(info->H_tree_clock){
			if ((PARM(TRANSISTOR_TYPE) == HVT) || (PARM(TRANSISTOR_TYPE) == NVT)) {
				...
				h = ceil(h); 			// the size of the buffer is an integer
				#endif
		...
		//fprintf(stdout, "Pipereg_clockcap: %f\tH_tree_clockcap: %f\tClockBufferCap: %f\n", pipereg_clockcap*1e15, H_tree_clockcap*1e15, ClockBufferCap*1e15);
		...
		fprintf(stdout, "Clock Pdyn (uW): %f\tClock Pleak (uW): %f\tClock Ptotal (uW): %f\tRouter_diagonal (um): %f\n", ClockEnergy*1e9*1e6, router->I_clock_static*Vdd*1e6, ClockEnergy*1e9*1e6 + router->I_clock_static*Vdd*1e6, info->router_diagonal);
