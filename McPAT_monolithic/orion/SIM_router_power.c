...
{	
	...
	fprintf(stdout, "Eavg: %g\tPbuf_static: %g\n", Eavg, Pbuf_static);
	#endif
	...
	fprintf(stdout, "Pxbar_dyn (uW): %g\tPxbar_static (uW): %g\tTotal (uW): %g\n", Pxbar_dyn*1e6, Pxbar_static*1e6, Pxbar*1e6);
	...
	if(info->sw_out_arb_model || info->sw_out_arb_model){
		...
		fprintf(stdout, "Psw_arbiter_dyn (uW): %g\tPsw_arbiter_static (uW): %g\tTotal (uW): %g\n", Psw_arbiter_dyn*1e6, Psw_arbiter_static*1e6, Psw_arbiter*1e6);
	}
	...
	if(info->n_v_channel > 1 && vc_allocator_enabled){
		...
		fprintf(stdout, "Pvc_arbiter_dyn (uW): %g\tPvc_arbiter_static (uW): %g\tTotal (uW): %g\n", Pvc_arbiter_dyn*1e6, Pvc_arbiter_static*1e6, Pvc_arbiter*1e6); 
	}
	fprintf(stdout, "Arbiter_dyn (uW): %g\tArbiter_static (uW): %g\tTotal (uW): %g\n", Pvc_arbiter_dyn*1e6+Psw_arbiter_dyn*1e6, Pvc_arbiter_static*1e6+Psw_arbiter_static*1e6, Pvc_arbiter*1e6+Psw_arbiter*1e6); 
	...
	//fprintf(stdout, "I_static: %g\tVdd: %g\tPeriod: %g\tEstatic: %g\n", router->I_static, Vdd, Period, Estatic);
	...
	if (plot_flag)
		fprintf(stdout, "Power (W): Crossbar:%g\tVC_allocator:%g\tSW_allocator:%g\tClock:%g\tTotal:%g\n", Pxbar, Pvc_arbiter, Psw_arbiter, Pclock, Ptotal); 
