...
double SIM_router_area(SIM_router_area_t *router_area)
{
	...
	fprintf(stdout, "Area (um^2): Crossbar:%g\t VCAllocator:%g\t SWAllocator:%g\t Total:%g\n", router_area->crossbar, router_area->vc_allocator, router_area->sw_allocator,  Atotal);	

#else
    fprintf(stderr, "Router area is only supported for 90nm, 65nm, 45nm, 32nm and 14nm\n");
#endif
	return Atotal;
}
