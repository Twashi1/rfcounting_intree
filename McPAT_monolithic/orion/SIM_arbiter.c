...
static double SIM_rr_arbiter_req_cap(double length)
{
		...
		return 2 * WdecNORn * CapInNOR + Wdecinvn * (CapOutINV + CapInINV) + length * Cmetal; 
	#endif
...
static double SIM_rr_arbiter_carry_cap()
{
	...
	#if(PARM(TECH_POINT) == 14)
		return 2 * WdecNORn * CapOutNOR + WdecNORn * CapInNOR; 
	#endif
...
static double SIM_rr_arbiter_carry_in_cap()
{
	...
	#if(PARM(TECH_POINT) == 14)
		return 2 * WdecNORn * CapInNOR + 2 * WdecNORn * CapOutNOR; 
	#endif
...
static double SIM_matrix_arbiter_req_cap(u_int req_width, double length)
{
	...
	#if(PARM(TECH_POINT) == 14)
		return (req_width - 1) * WdecNORn * CapInNOR + Wdecinvn * CapOutINV + Wdecinvn * CapInINV + WdecNORn * CapInNOR + length * Cmetal; 
	#endif
...
{
	#if(PARM(TECH_POINT) == 14)
		return req_width * WdecNORn * CapOutNOR; 
	#endif
	...
	if ((arb->model = arbiter_model) && arbiter_model < ARBITER_MAX_MODEL) {
		...
		switch (arbiter_model) {
			case RR_ARBITER:
				...
				#if(PARM(TECH_POINT) == 14)
					I_static = 6 * arb->req_width * WdecNORn * NOR2_TAB[0] +  	// NOR leakage
					2 * arb->req_width * Wdecinvn * INV2_TAB[0] +				// INV leakage 
					(arb->req_width * Wdff*DFF_TAB[0]);							// DFF leakage
				#endif
			...
			case MATRIX_ARBITER:
				...
				#if(PARM(TECH_POINT) == 14)
					I_static = 	(2 * arb->req_width - 1) * arb->req_width * WdecNORn * NOR2_TAB[0] +	// NOR leakage
					 			arb->req_width * Wdecinvn * INV2_TAB[0] +								// INV leakage
					 			arb->req_width * (arb->req_width - 1) / 2 * Wdff*DFF_TAB[0]; 			// DFF leakage
				#endif
